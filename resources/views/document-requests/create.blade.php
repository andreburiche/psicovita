<x-app-layout>
    <x-slot name="header">{{ __('Nova solicitação') }}</x-slot>
    ...
    <div class="py-8 sm:py-10">
        <div class="mx-auto max-w-6xl space-y-6 px-4 sm:px-6 lg:px-8" data-test="document-request-create">
            @include('document-requests.partials.patient-breadcrumb-trail', [
                'patient' => $patient,
                'current' => __('Nova solicitação'),
            ])

            <x-page-hero
                :title="__('Nova solicitação de documentos')"
                :subtitle="__('Pedido formal a escolas, empresas ou instituições — gera ofício em PDF.')"
                icon="document-text"
                iconTone="indigo"
            >
                <x-slot name="eyebrow">{{ __('Solicitação formal') }}</x-slot>
                <x-slot name="actions">
                    <a
            ...
            <div class="grid gap-6 lg:grid-cols-12 lg:items-start">
                <div class="space-y-6 lg:col-span-8">
                    <form
                        method="post"
                        action="{{ route('patients.document-requests.store', $patient) }}"
                    ...
                    >
                        ...
                        <div class="flex flex-col gap-4 rounded-2xl border border-slate-200/90 bg-white p-5 shadow-sm ring-1 ring-slate-100 dark:border-slate-700 dark:bg-slate-900/80 dark:ring-slate-700/60 sm:flex-row sm:items-center sm:justify-between">
                            <p class="flex items-start gap-2 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
                                ...
                                {{ __('Após salvar, acesse a solicitação para gerar o ofício em PDF e registrar o envio à instituição.') }}
                            </p>
                ...
                <div class="lg:col-span-4">
                    @include('document-requests.partials.create-sidebar', ['patient' => $patient])
                </div>
            </div>
        </div>
    </div>
